...
$rslt = queryMysql("SELECT * FROM rnfriends WHERE (user='$user' OR friend='$user') ORDER BY user");
...
if($num != null)
{
	for ($j = 0 ; $j < $num ; ++$j)
	{
		...
		if($row[5]==0 && $row[3]==$user){
			 $adding="Accept FriendShip ?";
			 $accept="add&accept=1";
			 }
			 elseif($row[5]==1 && ($row[3]==$user) || ($row[1]==$user)){
			 $adding="Unfriend This User!!";
			 $accept="remove";
			 }
			 if($row[5]==0 && $row[1]==$user){
		...
		//..........For Showing Relationship Status B/n Friends.......//
			...
			$follow = "Follow..!!";
echo<<<_END
		[<a id='followf' rel='$usern' onclick="clickyfriend('followf','$usern'); return false;" class='button green' href='followfriend.php?view=$usern&follow=follow&name=$name'>$follow</a>] <a onclick="clickyfriend('addf','$usern'); return false;" id='addf' rel='$usern' href='followfriend.php?view=$usern&add=$accept' class='button black' style='font-size:16px;'> &harr; $adding </a>
_END;
			}
			else {  	// unFollow The Friend 
			//$adding="UnFriend This User";
			$follow="UnFollow..!!";
echo<<<_END
		[<a  id='followf' rel='$usern' onclick="clickyfriend('followf','$usern'); return false;" class='button green' href='followfriend.php?view=$usern&follow=unfollow'>$follow</a>] <a onclick="clickyfriend('addf','$usern'); return false;" id='addf' rel='$usern' href='followfriend.php?view=$usern&add=$accept' class='button black' style='font-size:16px;'> &harr; $adding </a>
_END;
			}
		echo"<li></td></tr></table>";
	}
